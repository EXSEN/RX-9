...
//Calibration data
float cal_A = 456.7; //<--- QR CODE DATA
float cal_B = 68.9; //<--- QR CODE DATA
float CO2_ppm = 0.0;
float DEDT = 1.00; //<--- QR CODE DATA

//Auto Calibration coeff
int32_t MEIN = 60; //ABC
int32_t METI = 1440; //ABC, 1440 = 1 day, 2880 = 2 day
float EMF_max = 0; //ABC
...
void loop(){
  ...
  if(current_time - prev_time >= set_time){ //every 1 second, if you change set_time to 60, every 1 minute
    ...
    if(Sensor_status == 1){
      ...
      else if(CO2_ppm <= CO2_EARTH * 0.99){
        under_cut_count++; //ABC
...
void EMF_init(float EMF_data, float THER_data){
  ...
  cal_A = EMF_data + cal_B*log10(CO2_EARTH); //ABC
