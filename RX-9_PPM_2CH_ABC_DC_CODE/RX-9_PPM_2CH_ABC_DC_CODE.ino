...
void loop(){
  ...
  if(current_time - prev_time >= set_time){ //every 1 second, if you change set_time to 60, every 1 minute
    ...
    if(EMF_count<averaging_count_EMF){
      ...
      Serial.print(" Before T/C EMF: ");
      Serial.print(EMF_data);
      EMF_data = EMF_data+DEDT*(THER_ini-THER_data);
      Serial.print(" After T/C EMF: ");
      Serial.print(EMF_data);
      EMF_SUM = 0;
...
void PPM_CAL(float STEP_CO2){
  CO2_ppm = pow(10,((cal_A-(EMF_data)/(EMF_ini))/cal_B));
}
