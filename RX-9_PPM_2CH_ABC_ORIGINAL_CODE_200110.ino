...
unsigned int LOCK_delta = 40;
unsigned int LOCK_disable = 5;
unsigned int LOCK_timer = 1440;
...
void PPM_CAL(float EMF_data, float THER_data){
  ...
  if(CO2_ppm <= CO2_EARTH + CO2_BASE_Compensation){
    CO2_ppm = CO2_EARTH + CO2_BASE_Compensation;
  }
  else{
    // do nothing
  }
